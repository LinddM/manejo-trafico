...
export class Vehicle {
   ...
   */
  constructor(id: string, size: number, maxSpeed: number, accelaration: number){
    this.id = id
    this.size = size
  ...
  movement(){
    for (let i = 0; i < this.currentPosition.vehicles.length; i++){
      ...
      const lane: Array<Vehicle> = this.currentPosition.vehicles[i]
      const index = lane.indexOf(this)

      if (index === 0){
        if (this.currentPosition.direction.length > 0){
          // Defines next node to move and speed of movement
          let nextPosition: StreetNode = this.currentPosition.direction[Math.floor(Math.random() * this.currentPosition.direction.length)]
...
let vehicleTypes: Array<Vehicle> = [
